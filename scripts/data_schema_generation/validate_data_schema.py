...
def validate_metadata(metadata):
    ...
    if (
        not isinstance(metadata["core_concepts"], list)
        or len(metadata["core_concepts"]) == 0
    ):
...
def validate_table_structure(table, table_index):
    ...
    if empty_fields:
        print(
            f"❌ Table {table_index} ({table.get('name', 'unknown')}) empty fields: {', '.join(empty_fields)}"
        )
        return False
    ...
    for col_index, column in enumerate(table["columns"]):
        if "description" in column and column["description"] == "":
            print(
                f"❌ Table {table_index} column {col_index} ({column.get('name', 'unknown')}) has empty description"
            )
            return False
...
def validate_relationships(relationships):
    ...
    for index, relationship in enumerate(relationships):
        ...
        if missing_fields:
            print(
                f"❌ Relationship {index + 1} missing fields: {', '.join(missing_fields)}"
            )
            return False

        if empty_fields:
            print(
                f"❌ Relationship {index + 1} empty fields: {', '.join(empty_fields)}"
            )
            return False
...
def validate_educational_context(data):
    ...
    if tables_without_purpose:
        print(
            f"❌ Tables without educational purpose: {', '.join(tables_without_purpose)}"
        )
        return False
...
def count_enhancement_completion(data):
    ...
    print(
        f"📊 Tables with descriptions: {len([t for t in tables if t.get('description')])}"
    )
    print(
        f"📊 Tables with educational purposes: {len([t for t in tables if t.get('educational_purpose')])}"
    )
    print(
        f"📊 Relationships with descriptions: {len([r for r in relationships if r.get('description')])}"
    )
...
def validate_data_schema(dataset_name):
    ...
    if all(validations):
        ...
        print(
            f"📊 Structure: {data['metadata']['total_tables']} tables, {data['metadata']['total_records']:,} records"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Validate enhanced data schema JSON file"
    )
    parser.add_argument(
        "--dataset", required=True, help="Dataset name (e.g., data_jobs)"
    )
