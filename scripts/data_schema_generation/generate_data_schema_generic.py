...
def load_existing_artifacts(dataset_name):
    ...
    exploration_file = (
        Path(__file__).parent / f"initial_exploration_{dataset_name}.json"
    )
...
def introspect_database(helper, artifacts):
    ...
    if "table_queries" in artifacts:
        creation_queries = {
            name: data["query"]
            for name, data in artifacts["table_queries"]["tables"].items()
        }
        ...
        for column in table["columns"]:
            if column["foreign_key"]:
                ...
                if "References" in column["foreign_key"]:
                    ref_parts = (
                        column["foreign_key"].replace("References ", "").split(".")
                    )
                    if len(ref_parts) == 2:
...
def generate_schema_notes(tables_info):
    ...
    # Count foreign keys
    fk_count = sum(
        1 for table in tables_info for col in table["columns"] if col["foreign_key"]
    )
    if fk_count > 0:
        notes.append(
            f"Database contains {fk_count} foreign key relationships ensuring referential integrity"
        )
    ...
    if junction_tables:
        notes.append(
            f"Junction tables detected: {', '.join(junction_tables)} - implementing many-to-many relationships"
        )
...
def generate_agent_prompt(dataset_name, output_path, schema_data, artifacts):
    ...
    print(
        f"📊 Structure: {schema_data['metadata']['total_tables']} tables, {schema_data['metadata']['total_records']:,} records"
    )
    ...
    print("  • Database introspection (11 tables discovered)")
    print("  • Column types, constraints, and nullability")
    print("  • Foreign key relationships via naming conventions")
    print("  • Sample data (3 rows per table)")
    print(f"  • Creation queries from table_creation_queries_{dataset_name}.json")
    print("  • Educational metadata preserved")

    # What Needs Enhancement
    print("\n🎯 AGENT ENHANCEMENT NEEDED:")
    print(
        "Your task is to enhance the following BLANK fields with intelligent, contextual content:"
    )
    ...
    print("")
    print("1. METADATA ENHANCEMENTS:")
    print("   • metadata.description: Write a comprehensive overview of the database")
    print("   • metadata.source: Identify the original data source")
    print("   • metadata.use_cases: List 3-5 educational use cases")
    print("")
    print("2. TABLE ENHANCEMENTS:")
    print("   • tables[].description: Business context for each table")
    print("   • tables[].educational_purpose: Learning objectives per table")
    print("")
    print("3. COLUMN ENHANCEMENTS:")
    print("   • tables[].columns[].description: Semantic meaning of each column")
    print("   • Focus on business context, not just technical details")
    print("")
    print("4. RELATIONSHIP ENHANCEMENTS:")
    print("   • relationships[].description: Explain the business relationship")
    print("")
    print("5. SCHEMA NOTES:")
    print("   • Fill in the 3 empty strings with insights about:")
    print("     - Normalization approach and design decisions")
    print("     - Business logic and domain modeling")
    print("     - Optimization and performance considerations")

    # Next Steps
    print("\n🚀 NEXT STEPS:")
    print(f"1. Open the generated schema file: {output_path}")
    print('2. Enhance ALL empty string fields ("") with meaningful content')
    print("3. Use the sample data and creation queries for context")
    print("4. Ensure descriptions are educational and business-focused")
    print(
        f"5. Maintain the educational objectives for Week {metadata.get('target_week', 'N/A')} SQL curriculum"
    )

    print("\n💡 TIPS:")
    print("   • Use the sample_data arrays to understand content patterns")
    print("   • Reference the creation_query fields for business logic")
    print("   • Focus on educational value for JOIN practice")
    print("   • Make descriptions helpful for SQL learning, not just documentation")

    print("\n🎯 SUCCESS CRITERIA:")
    print("   • No empty string fields remain")
    print("   • All descriptions are educational and contextual")
    print("   • Schema serves as comprehensive learning resource")
    print(
        f"   • Ready for SQL curriculum Week {metadata.get('target_week', 'N/A')} exercises"
    )
...
def generate_data_schema(dataset_name):
    ...
    print(
        f"  📝 Needs Agent Enhancement: {len(schema_data['agent_enhancement_needed']['fields_to_enhance'])} field types"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate comprehensive data schema documentation"
    )
    parser.add_argument(
