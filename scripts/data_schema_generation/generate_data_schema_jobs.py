...
def get_table_creation_queries():
    ...
    queries_file = os.path.join(
        os.path.dirname(__file__), "table_creation_queries_jobs.json"
    )
...
def generate_data_jobs_schema():
    ...
    print(
        f"  📈 Total Records: {int(sum(table['row_count'] for table in tables_info)):,}"
    )
    ...
    for table in tables_info:
        print(
            f"  • {table['name']}: {int(table['row_count']):,} rows, {len(table['columns'])} columns"
        )
