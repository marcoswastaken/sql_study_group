...
def generate_exercise_key(dataset, week_num, exercises):
    ...
    return {
        "metadata": {
            ...
            "focus_topics": ["INNER JOIN", "LEFT JOIN", "Table aliases", "Primary/Foreign keys"],
...
def main():
    parser = argparse.ArgumentParser(description='Generate SQL exercises for a specific week')
    ...
    print(f"🎯 Topics covered: {', '.join(set(topic for ex in exercises for topic in ex['topics']))}")
