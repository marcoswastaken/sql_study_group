...
def execute_solution(db_manager, solution, limit=20):
    ...
    if "LIMIT" not in last_line.upper():
        ...
        if "LIMIT" in last_line.upper():
            ...
            solution_lines[-1] = re.sub(
                r"LIMIT\s+\d+", f"LIMIT {limit}", last_line, flags=re.IGNORECASE
            )
...
def generate_markdown_report(exercise_key, dataset):
    ...
    markdown_content.append(
        f"**Description:** {exercise_key['metadata']['description']}"
    )
    markdown_content.append(f"**Database:** {exercise_key['metadata']['database']}")
    markdown_content.append(
        f"**Total Exercises:** {exercise_key['metadata']['total_exercises']}"
    )
    markdown_content.append(
        f"**Focus Topics:** {', '.join(exercise_key['metadata']['focus_topics'])}"
    )
    markdown_content.append("")
    markdown_content.append(
    ...
    for exercise in exercise_key["exercises"]:
        ...
        try:
            ...
            if result["status"] == "success":
                markdown_content.append("✅ **Query executed successfully**")
                markdown_content.append(
                ...
                if result["data"] is not None and not result["data"].empty:
                    ...
                    markdown_content.append("")
                    table_markdown = format_dataframe_as_markdown_table(
                        result["data"], max_rows=20
                    )
                    markdown_content.append(table_markdown)
                else:
                    markdown_content.append("*No data returned by the query.*")
            else:
                markdown_content.append("❌ **Query failed**")
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate markdown report for exercise solutions"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output", type=str, help="Output markdown file path (optional)"
    )
    ...
    try:
        ...
        if args.output:
            ...
            output_path = (
                ...
                / "reports"
                / f"week_{week_num}"
                / f"week_{week_num}_key_report_{dataset_name}{version}.md"
            )
