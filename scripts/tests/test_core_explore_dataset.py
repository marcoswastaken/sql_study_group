...
from core.explore_dataset import DatasetExplorer
...
class TestDatasetExplorer(unittest.TestCase):
    ...
    def test_column_type_classification(self):
        ...
        mock_data = {
            ...
            'categorical_col': ['A', 'A', 'B', 'B', 'C'] * 20,  # 100 items, 3 unique (3% unique)
            'text_col': [f'unique_text_{i}' for i in range(100)],  # 100 unique items (100% unique)
    ...
    def test_potential_id_field_detection(self):
        ...
        mock_data = {
            ...
            'random_unique': np.random.choice(range(1000, 2000), 100, replace=False),  # Should be detected as ID
            'non_unique': [1, 2, 3] * 33 + [1],  # Should NOT be detected as ID
            'negative_nums': range(-50, 50),  # Should NOT be detected as ID (has negatives)
        }
    ...
    def test_foreign_key_detection(self):
        ...
        company_names = ['Company_A', 'Company_B', 'Company_C', 'Company_D', 'Company_E']
        mock_data = {
            'id': range(1, 1001),
            ...
            'too_few_categories': ['X', 'Y'] * 500,  # Should NOT be FK (too few categories)
        }
        ...
        self.assertFalse(columns['too_few_categories'].get('potential_foreign_key', False))
    ...
    def test_data_quality_analysis(self):
        ...
        self.assertLess(data_quality['completeness_score'], 100)  # Should be less than 100% due to nulls
    ...
    def test_normalization_opportunities(self):
        ...
        suggested_tables = [opp['suggested_table'] for opp in relationships['normalization_opportunities']]
    ...
    def test_single_column_dataset(self):
        ...
        assessment = results['educational_assessment']
        self.assertIn('Few columns limit complexity', ' '.join(assessment['limitations']))
    
    def test_all_null_column(self):
        """Test handling of columns with all null values."""
        mock_data = {
            'normal_col': range(100),
            'all_null_col': [None] * 100
        }
        ...
        self.assertEqual(columns['all_null_col']['sample_values'], ['(all null values)'])
    ...
    def test_sample_values_extraction(self):
        ...
        mock_data = {
            ...
            'many_categories': [f'cat_{i}' for i in range(102)],  # Many unique values (102 items)
            'long_text': ['This is a very long text that should be truncated because it exceeds the limit'] * 102  # 102 items
        }
    ...
    def test_with_small_real_dataset(self):
        ...
        try:
            ...
            if success and explorer.df is not None:
                ...
                self.assertIn('educational_assessment', results)
                self.assertIsInstance(results['educational_assessment']['sql_suitability_score'], (int, float))
                
                print(f"✅ Real dataset test passed:")
                print(f"   Records: {results['metadata']['total_records']:,}")
                print(f"   Columns: {results['metadata']['total_columns']}")
                print(f"   SQL Score: {results['educational_assessment']['sql_suitability_score']}/10")
