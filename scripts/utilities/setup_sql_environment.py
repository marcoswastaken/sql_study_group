...
def setup_sql_environment():
    ...
    conn.execute(
        "CREATE TABLE IF NOT EXISTS companies (company_id INTEGER PRIMARY KEY, company_name VARCHAR(255))"
    )
    conn.execute("INSERT INTO companies SELECT * FROM companies_df")

    # Create locations table
    locations_df = (
        df[["job_location", "job_country"]].drop_duplicates().reset_index(drop=True)
    )
    locations_df["location_id"] = locations_df.index + 1
    locations_df = locations_df[["location_id", "job_location", "job_country"]]
    conn.execute(
        "CREATE TABLE IF NOT EXISTS locations (location_id INTEGER PRIMARY KEY, job_location VARCHAR(255), job_country VARCHAR(100))"
    )
    ...
    conn.execute(
        "CREATE TABLE IF NOT EXISTS platforms (platform_id INTEGER PRIMARY KEY, platform_name VARCHAR(255))"
    )
    ...
    location_mapping = dict(
        zip(locations_df["job_location"], locations_df["location_id"])
    )
    platform_mapping = dict(
        zip(platforms_df["platform_name"], platforms_df["platform_id"])
    )
    ...
    skills_df = pd.DataFrame(
        skills_data, columns=["skill_id", "skill_name", "skill_category"]
    )
    conn.execute(
        "CREATE TABLE IF NOT EXISTS skills (skill_id INTEGER PRIMARY KEY, skill_name VARCHAR(100), skill_category VARCHAR(50))"
    )
    ...
    job_skills_data = []
    for job_id in range(
        1, min(10000, len(jobs_final) + 1)
    ):  # Limit to first 10k jobs for demo
