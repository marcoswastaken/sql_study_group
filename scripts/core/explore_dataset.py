...
class DatasetExplorer:
    ...
    def create_database(self, database_path: str) -> bool:
        """
        Create normalized database from the dataset.
        ...
        try:
            print(f"🗄️  Creating normalized database: {database_path}")
            ...
            companies_df = self.df[['company_name']].drop_duplicates().reset_index(drop=True)
            ...
            locations_df = self.df[['job_location', 'job_country']].drop_duplicates().reset_index(drop=True)
            ...
            platforms_df = self.df[['job_via']].drop_duplicates().reset_index(drop=True)
...
def main():
    ...
    parser.add_argument(
        "--create-database", 
        action="store_true", 
        help="Create normalized database from the dataset"
    )
    ...
    if args.create_database:
        dataset_name = args.dataset.split('/')[-1]
        # Remove 'data_' prefix if already present to avoid duplication
